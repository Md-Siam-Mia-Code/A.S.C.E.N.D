...
function populateAppList(data) {
  ...
  apps.sort().forEach((pkg) => {
    ...
    const info = debloatDB[pkg] || {};
    const appName = info.name || pkg.split(".").pop();
    item.innerHTML = `<div class="app-item-name" title="${pkg}">${appName}</div>`;
...
function showAppDetails(pkg) {
  ...
  const info = debloatDB.find((app) => app.id === pkg) || {
    id: pkg,
    name: pkg.split(".").pop(),
    description:
